...
void draw(){
  ...
  for(int i=1; i<lines.length; i++){
    ...
    arrayList_xAxis.add(satmScore);
    arrayList_yAxis.add(satvScore);
    
  }
  drawAxes(tests[0],tests[1]);
  drawScales();
  plot();
}
...
void drawScales(){
  ...
  for(int k=0; k<5; k++){
    ...
    text((k+1)*Collections.max(arrayList_xAxis)/5 , 45+(k+1)*(width-150)/5, height-20);
    text("-", 50, height-50-(k+1)*(height-150)/5);
    text((k+1)*Collections.max(arrayList_yAxis)/5 , 10, height-50-(k+1)*(height-150)/5);
  }
}
void plot(){
